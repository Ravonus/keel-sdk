...
/** SHA-256 for opaque data: documents where WebCrypto is intentionally unavailable. */
export function sha256Fallback(input) {
  ...
  for (let block = 0; block < paddedLength; block += 64) {
    for (let index = 0; index < 16; index += 1) {
      schedule[index] = paddedView.getUint32(block + index * 4, false);
    }
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + upper + choice + ROUND_CONSTANTS[index] + schedule[index]) >>> 0;
...
export async function browserSha256(bytes) {
