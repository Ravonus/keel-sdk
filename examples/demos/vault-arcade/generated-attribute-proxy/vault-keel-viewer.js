...
const runtimeData = globalThis.__OCA_RUNTIME__;
const contextData = runtimeData?.context ?? globalThis.__OCA_CONTEXT__ ?? {
...
const verificationFixturesAllowed = allowCollectionVerificationFixtureQuery(location.hostname, globalThis.__OCA_TEST_MODE__ === true);
...
const collectionFixture = (scenario) => {
  ...
  return {
    ...
    chainId: String(globalThis.__OCA_CONTEXT__?.chainId ?? 11155111),
    blockNumber: globalThis.__OCA_CONTEXT__?.blockNumber ?? "9123456",
    blockHash: globalThis.__OCA_CONTEXT__?.blockHash ?? `0x${"12".repeat(32)}`,
    observationBlockNumber: "9123455", observationBlockHash: `0x${"18".repeat(32)}`,
    collection: "0x2222222222222222222222222222222222222222", tokenId: globalThis.__OCA_CONTEXT__?.tokenId ?? "1",
...
const collectionVerificationInput = globalThis.__OCA_CONTEXT__?.collectionVerification
...
const baseVerification = evaluateVaultVerification(runtimeData, globalThis.__OCA_CONTEXT__, verificationScenario);
...
const verificationUI = presentationMode === "gallery"
  ? Object.freeze({
    ready() {
      parent.postMessage({ protocol: "oca-viewer-verification@1", action: "preview-ready", state: verification.state }, "*");
    },
    fail() {},
  })
  : mountVerificationUI(verification, runtimeData, globalThis.__OCA_CONTEXT__);
...
const selectedWeapon = Object.entries(weaponAttributeCatalog.weapons).find(([, definition]) => definition.assetId?.toLowerCase() === assetId)?.[0]
  ?? (globalThis.__OCA_CONTEXT__ == null
...
function drawGalleryPresentation(time) {
  ...
  if (!firstFrameReady) {
    ...
    globalThis.__OCA_THUMBNAIL__?.ready("vault-orb-gallery");
...
function draw(time) {
  ...
  if (!firstFrameReady) {
    ...
    document.body.classList.add("ready");
    document.documentElement.dataset.vaultCharacterReady = "true";
    globalThis.__OCA_THUMBNAIL__?.ready("vault-orb");
