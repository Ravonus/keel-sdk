 ...
 * composition path (that runs through __KEEL__/__OCA_CONTENT__/the gateway,
...
function thumbnailBootstrap(artifact: ResolvedArtifact): string {
  ...
  return `(()=>{"use strict";const policy=${serialized};const started=performance.now();let timer;let stopTimer;let initialized=false;const emit=(action,label)=>parent.postMessage({protocol:"oca-thumbnail-capture@1",action,label:String(label||policy?.label||"hero").slice(0,64),elapsedMs:Math.max(0,Math.round(performance.now()-started))},"*");const stop=label=>{clearTimeout(timer);clearTimeout(stopTimer);emit("stop",label)};const capture=label=>{emit("capture",label);if(policy?.durationMs)stopTimer=setTimeout(()=>stop(label),policy.durationMs)};const init=label=>{if(initialized)return;initialized=true;emit("init",label);if(policy?.mode==="after-init")timer=setTimeout(()=>capture(label),policy.delayMs||0)};const api=Object.freeze({protocol:"oca-thumbnail-capture@1",init,capture,ready:capture,stop,after:(delayMs,label)=>{const delay=Number(delayMs);if(!Number.isFinite(delay)||delay<0||delay>30000)throw new RangeError("Thumbnail delay must be from 0 through 30000ms.");clearTimeout(timer);timer=setTimeout(()=>capture(label),delay)}});Object.defineProperty(globalThis,"__OCA_THUMBNAIL__",{value:api,enumerable:true,configurable:false,writable:false});for(const [name,action] of [["OCA_THUMBNAIL_INIT",init],["OCA_THUMBNAIL_STOP",stop]]){let value=false;Object.defineProperty(globalThis,name,{enumerable:true,configurable:false,get:()=>value,set:next=>{value=Boolean(next);if(value)action()}})}if(policy?.mode==="time")addEventListener("DOMContentLoaded",()=>{timer=setTimeout(()=>capture(),policy.delayMs||0)},{once:true})})();`;
}
...
function gatewayPayload(artifact: ResolvedArtifact): GatewayPayload {
  ...
  for (const route of gateway.routes) {
    ...
    // __OCA_CONTENT__ is an ID-addressed reader API. Keep the raw resource ID
...
function headPayload(artifact: ResolvedArtifact, csp: string, runtimeContext?: RuntimeContext, consumer = false): string {
  ...
  const verificationChrome = `(() => {
    ...
    addEventListener("message", (event) => {
      ...
      if (command?.protocol === "oca-viewer-verification@1" && command.action === "set-chrome") apply(command.chrome);
    });
  })();`;
