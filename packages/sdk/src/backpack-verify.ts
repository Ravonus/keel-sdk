 ...
 *
 *   const report = await verifyBackpackToken({ rpcUrl, backpack, tokenId });
...
export type BackpackCustody = "None" | "Sealed" | "Frozen" | "Hollow" | "Compromised";
export type BackpackLane = "Unproven" | "Attested" | "Committed" | "Native";
 ...
 */
export type BackpackCarriage = "Unknown" | "Linked" | "Inline" | "Hybrid";
...
export interface VerifyBackpackOptions {
...
const CARRIAGE: readonly BackpackCarriage[] = ["Unknown", "Linked", "Inline", "Hybrid"];

const CUSTODY: readonly BackpackCustody[] = ["None", "Sealed", "Frozen", "Hollow", "Compromised"];
 ...
 */
export async function verifyBackpackToken(options: VerifyBackpackOptions): Promise<BackpackReport> {
  ...
  const carriage = await (async (): Promise<BackpackCarriage> => {
